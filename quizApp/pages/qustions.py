...
stuid = st.session_state.current_user.Uid
...
if(exam[3] == "exam"):
    ...
    if st.session_state.get("exam_finished"):
        st.error("Exam time has ended! Submitting your answers...")
        time.sleep(2)
        st.session_state.pop("exam_finished", None)
...
if index >= len(qustions):
    ...
    st.session_state.pop("timer_start", None)
    st.session_state.pop("examObj", None)
    st.switch_page("pages/result.py")
else :
    if exam[3] == "Question":
        remain = Qus_countdown_timer(exam[4])
        if st.session_state.get("timer_done"):
            st.error("Time's up for this question! Moving to next question...")
            time.sleep(1)
            st.session_state.pop("timer_done", None)
            st.session_state.pop("timer_start", None)
            st.session_state.index = index+1
            db.insert("answers", {"QusID":qustions[index][0], "AttID": attid, "ChID":None, "correctnes":False})
            st.rerun()
    ...
    for choice in choices:
        ...
        if st.button(choiceHeader):
            ...
            if "timer_start" in st.session_state:
                st.session_state.pop("timer_start")
            st.rerun()
time.sleep(1)
st.rerun()